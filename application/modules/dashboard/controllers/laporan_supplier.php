...
class laporan_supplier extends CI_Controller {
	...
	function index($uri=0)
	{
		if($this->session->userdata("logged_in")!="")
		{
			...
			$d['dt_retrieve'] = $this->app_load_data_model->indexs_data_laporan_supplier($GLOBALS['site_limit_medium'],$uri);
			
			//load view
			//parsing data ke view
 			$this->load->view($GLOBALS['site_theme']."/bg_header");
 			$this->load->view($GLOBALS['site_theme']."/bg_left",$d);
 			$this->load->view($GLOBALS['site_theme']."/laporan_supplier/bg_home");

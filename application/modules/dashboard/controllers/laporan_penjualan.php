...
class laporan_penjualan extends CI_Controller {
	...
	function index($uri=0)
	{
		if($this->session->userdata("logged_in")!="")
		{
			...
			$d['dt_retrieve'] = $this->app_load_data_model->indexs_data_laporan_penjualan($GLOBALS['site_limit_medium'],$uri);
			
			//load view
			//parsing data ke view
 			$this->load->view($GLOBALS['site_theme']."/bg_header");
 			$this->load->view($GLOBALS['site_theme']."/bg_left",$d);
 			$this->load->view($GLOBALS['site_theme']."/laporan_penjualan/bg_home");
	...
	function detail($id_param)
	{
		if($this->session->userdata("logged_in")!="")
		{
			...
			//parsing data ke view
 			$this->load->view($GLOBALS['site_theme']."/bg_header");
 			$this->load->view($GLOBALS['site_theme']."/bg_left",$d);
 			$this->load->view($GLOBALS['site_theme']."/laporan_penjualan/bg_detail");
 			$this->load->view($GLOBALS['site_theme']."/bg_footer");
		}
		else
		{
	...
	function cetak_excel($id_param)
	{
		if($this->session->userdata("logged_in")!="")
		{
			...
			//load view
			//parsing data ke view
 			$this->load->view($GLOBALS['site_theme']."/laporan_penjualan/cetak",$d);
		}
		else
		{
			//arahkan ke halaman login
	...
	function cetak_pdf($id_param)
	{
		if($this->session->userdata("logged_in")!="")
		{
			...
			$this->load->helper("gen_pdf_helper");
 			$html = $this->load->view($GLOBALS['site_theme']."/laporan_penjualan/cetak",$d, true);
